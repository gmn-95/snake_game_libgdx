...
public class Snake {
    ...
    private static final int TAMANHO_CORPO_INICIAL = 2; // Tamanho de cada parte do corpo
    ...
    protected void adicionaParteCorpo(int xAntesDeAtualizar, int yAntesDeAtualizar){
        ParteCorpo corpo = new ParteCorpo(x - (TAMANHO_CORPO * partesCorpo.getFirst().getX()), y, new Sprite(texturaCorpo));
        partesCorpo.add(corpo);
    }

    public void adicionaParteCorpo(){
        isAdiciona = true;
    }

    protected void atualizaMovimentacaoDaCobra(int xAntesDeAtualizar, int yAntesDeAtualizar){
        if(isAdiciona){
            adicionaParteCorpo(xAntesDeAtualizar, yAntesDeAtualizar);
            isAdiciona = false;
        }

        if (Gdx.graphics.getFrameId() % 12 == 0){// Ajusta a frequência de movimento das partes do corpo
            if(!partesCorpo.isEmpty()){
                ParteCorpo parteCorpo = partesCorpo.remove(0);
                parteCorpo.setX(xAntesDeAtualizar);
                parteCorpo.setY(yAntesDeAtualizar);
//                    parteCorpo.getSprite().setRotation(rotacao);
    ...
    protected void defineDirecaoERotacao(){
        // Atualiza a direção baseada nas teclas pressionadas
    ...
    protected void moveNaDirecaoEscolhida(){
        if (direcao == Input.Keys.RIGHT) {
            x += 2;
        } else if (direcao == Input.Keys.LEFT) {
            x -= 2;
        } else if (direcao == Input.Keys.UP) {
            y += 2;
        } else if (direcao == Input.Keys.DOWN) {
            y -= 2;
        }
    }
